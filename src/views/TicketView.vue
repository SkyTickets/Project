...
onMounted(async () => {
  ...
  if (booking.value.bUser !== userStore.currentUser.uId && userStore.currentUser.uRole !== 'Менеджер') {
...
const classColor = computed(() => ({
  'Эконом': '#2196F3', 'Комфорт': '#4CAF50',
  'Бизнес': '#FF9800', 'Первый класс': '#9C27B0',
...
// Итого: тариф + услуги (зеркалит логику SendEmail.cs)
      ...
      <div class="ticket" :class="{ cancelled: isCancelled }">
        ...
        <div class="ticket-header">
          <div class="airline-info">
            <div class="airline-logo-wrap">
              <img v-if="imageUrl" :src="imageUrl" :alt="ticket.fAirline" class="airline-logo" @error="onImageError" />
        ...
        <div class="details-section">
          <div class="detail-group">
            ...
            <div class="detail-latin" v-if="ticket.tPassengerLatin">{{ ticket.tPassengerLatin }}</div>
          </div>
          <div class="detail-group">
...
.loading {
  ...
  justify-content: center; min-height: 50vh; gap: 16px; color: #666;
}
.spinner {
...
.ticket-wrapper { max-width: 680px; margin: 0 auto; }

.actions-bar {
  display: flex; justify-content: space-between;
  align-items: center; margin-bottom: 20px;
}
.btn-back { background: none; border: none; color: #2196F3; font-size: 15px; cursor: pointer; padding: 8px 0; }
.btn-print { background: white; border: 1px solid #ddd; border-radius: 8px; padding: 8px 16px; font-size: 14px; cursor: pointer; color: #333; }
...
.airline-logo-placeholder {
  ...
  font-size: 22px; font-weight: 700; color: #1565C0;
}
.airline-name { font-size: 18px; font-weight: 600; }
.ticket-num { font-size: 13px; opacity: 0.75; margin-top: 2px; }
.status-badge {
  padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600;
  background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3);
}
.status-badge.забронирован { background: rgba(255,193,7,0.35); }
.status-badge.оплачен { background: rgba(76,175,80,0.35); }
.status-badge.отменен { background: rgba(244,67,54,0.35); }

.divider-line { height: 1px; background: #e8ecf0; }

/* Маршрут */
.route-section { display: flex; align-items: center; padding: 28px 28px 20px; gap: 12px; }
.route-point { flex: 1; }
.route-point.right { text-align: right; }
.route-time { font-size: 40px; font-weight: 700; color: #0d0d0d; line-height: 1; font-variant-numeric: tabular-nums; }
.route-airport { font-size: 15px; color: #333; margin-top: 6px; font-weight: 500; }
.route-date { font-size: 13px; color: #888; margin-top: 3px; }
.route-middle { flex: 0 0 140px; text-align: center; }
.route-duration { font-size: 13px; color: #666; margin-bottom: 8px; }
.route-line { display: flex; align-items: center; gap: 4px; }
.route-dot { width: 8px; height: 8px; border-radius: 50%; background: #1976D2; flex-shrink: 0; }
.route-arrow { flex: 1; text-align: center; font-size: 18px; color: #1976D2; }
.route-direct { font-size: 12px; color: #4CAF50; margin-top: 6px; font-weight: 500; }

/* Линия отрыва */
.tear-line { display: flex; align-items: center; position: relative; height: 2px; background: transparent; }
.tear-circle { width: 24px; height: 24px; background: #f0f4f8; border-radius: 50%; position: absolute; top: 50%; transform: translateY(-50%); z-index: 1; }
.tear-circle.left { left: -12px; }
.tear-circle.right { right: -12px; }
.tear-dashes { flex: 1; border-top: 2px dashed #ddd; margin: 0 20px; }

/* Детали */
.details-section {
  display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 0; padding: 24px 28px 20px;
}
.detail-group { padding: 0 16px 0 0; border-right: 1px solid #f0f0f0; }
.detail-group:last-child { border-right: none; padding-right: 0; }
.detail-group:not(:first-child) { padding-left: 16px; }
.detail-label { font-size: 10px; font-weight: 700; color: #aaa; letter-spacing: 0.8px; text-transform: uppercase; margin-bottom: 6px; }
.detail-value { font-size: 15px; font-weight: 600; color: #1a1a1a; }
.detail-latin { font-size: 12px; color: #888; margin-top: 3px; font-style: italic; letter-spacing: 0.5px; }
.class-badge { display: inline-block; color: white; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 700; }
.price { font-size: 18px; color: #1565C0; }
.price-group { text-align: right; border-right: none; }
...
.services-section .detail-label { margin-bottom: 12px; }
...
.service-name { flex: 1; }
.service-price { font-weight: 600; color: #374151; }
.services-subtotal {
  display: flex; justify-content: space-between; align-items: center;
...
/* Итого */
.total-section { padding: 0 28px 28px; }
.total-bar {
  display: flex; justify-content: space-between; align-items: center;
  background: #1e1b4b;
  border-radius: 10px;
...
.total-label { font-size: 14px; color: #a5b4fc; }
.total-price { font-size: 24px; font-weight: 800; color: white; }
...
.info-note { text-align: center; color: #999; font-size: 13px; margin-top: 16px; }

@media (max-width: 600px) {
  .details-section { grid-template-columns: 1fr 1fr; row-gap: 20px; }
